...
class Framework(cmd.Cmd):
    ...
    def add_targets(self, location, vendor=None, product_name=None, version=None, extracted_location=None, sandbox_id=None, mute=False):
        ...
        data = dict(
            location = self.to_unicode(location),
            vendor = self.to_unicode(vendor),
    ...
    def is_archive(self, file_name):
        try:
            return tarfile.is_tarfile(file_name) or zipfile.is_zipfile(file_name)
        except:
            return False

    def extract_archive(self, file_name, output_folder):
        try:

            if not os.path.exists(output_folder):
                self.output("Path %s does not exist creating..." % output_folder)
                os.makedirs(output_folder)

            if tarfile.is_tarfile(file_name):
                tarfile.open(file_name).extractall(output_folder)

            if zipfile.is_zipfile(file_name):
                zipfile.ZipFile(file_name).extractall(output_folder)

            return True
        except Exception as ex:
            return False

    def add_virus(self, product=None, version=None, engine=None, engine_version=None, update=None, description=None, mute=False):
        data = dict(
            product = self.to_unicode(product),
            version = self.to_unicode(version),
            engine = self.to_unicode(engine),
            engine_version = self.to_unicode(engine_version),
            update_version = self.to_unicode(update),
            description = self.to_unicode(description)
        )

        rowcount = self.insert('virus_scan', data.copy(), data.keys())
        #if not mute: self._display(data, rowcount, '[virus_scan] %s', data.keys())
        return rowcount

    def add_qa_issue(self, product=None, version=None, file_name=None, location=None, line_number=None, description=None, note='', mute=False):
        data = dict(
            product = self.to_unicode(product),
            version = self.to_unicode(version),
            file_name = self.to_unicode(file_name),
            location = self.to_unicode(location),
            line_number = self.to_unicode(line_number),
            description = self.to_unicode(description),
            note = self.to_unicode(note)
            )

        rowcount = self.insert('qa_issue', data.copy(), data.keys())
        #if not mute: self._display(data, rowcount, '[virus_scan] %s', data.keys())
        return rowcount

    def add_url(self, product=None, version=None, method=None, url=None, data=None, mute=False):
        data = dict(
            product = self.to_unicode(product),
            version = self.to_unicode(version),
            method = self.to_unicode(method),
            url = self.to_unicode(url),
            data = self.to_unicode(data)
            )

        rowcount = self.insert('url', data.copy(), data.keys())
        #if not mute: self._display(data, rowcount, '[virus_scan] %s', data.keys())
        return rowcount

    def add_cve(self, product=None, version=None, cve_number=None, description=None, published=None, cvss=None, mute=False):
        data = dict(
            product = self.to_unicode(product),
            version = self.to_unicode(version),
            cve_number = self.to_unicode(cve_number),
            description = self.to_unicode(description),
            published = self.to_unicode(published),
            cvss = self.to_unicode(cvss)
        )

        rowcount = self.insert('cve', data.copy(), data.keys())
        #if not mute: self._display(data, rowcount, '[virus_scan] %s', data.keys())
        return rowcount

    def add_sandbox_result(self, product=None, version=None, description=None, severity=None, note='', mute=False):
        data = dict(
            product = self.to_unicode(product),
            version = self.to_unicode(version),
            description = self.to_unicode(description),
            severity = self.to_unicode(severity)
        )

        rowcount = self.insert('sandbox_result', data.copy(), data.keys())
        #if not mute: self._display(data, rowcount, '[virus_scan] %s', data.keys())
        return rowcount

    def add_reported_change(self, product=None, version=None, object_type=None, action=None, change_data=None, mute=False):
        data = dict(
            product = self.to_unicode(product),
            version = self.to_unicode(version),
            object_type = self.to_unicode(object_type),
            action = self.to_unicode(action),
            change_data = self.to_unicode(change_data)
        )
        rowcount = self.insert('reported_change', data.copy(), data.keys())
        return rowcount
    ...
    def show_dashboard(self, *params):
    ...
    def show_cve(self, *param):
        suffix = ''
        if param[0] != '':
            params = param[0].split('-')
            if len(params) > 1 :
                suffix = " WHERE ROWID >= %s AND ROWID <= %s" % (params[0], params[1])
            else:
                suffix = " WHERE ROWID = %s" % param

        results = self.query('SELECT ROWID, * FROM cve%s' % suffix)
        for r in results:
            print("Id: %d" % r[0])
            print("Product: %s" % r[1])
            print("Version: %s" % r[2])
            print("CVE Number: %s" % r[3])
            print("Published: %s" % r[5])
            print("Description: %s" % r[4])
            print("CVSS: %s" % r[6])
            print("Module: %s" % r[7])
            print('')

    def show_reported_change(self, *param):
        suffix = ''
        if param[0] != '':
            params = param[0].split(' ')
            suffix = " WHERE object_type = '%s'" % (params[0])
            
            if len(params) > 1 and  "," not in params[1]:
                suffix += " AND action = '{}'".format(params[1])

            if (len(params) > 2 and params[2] != '') or len(params) > 1 and "," in params[1]:
                tmp = params[1]
                if len(params) > 2: tmp = params[2]
                page, per_page = tmp.split(",")
                page = int(page)
                per_page = int(per_page)
                start_row = (page - 1) * per_page 
                if start_row == 0:
                    suffix += " LIMIT {}".format(per_page)
                else:
                    suffix += " LIMIT {}, {}".format(per_page, start_row)

        results = self.query('SELECT ROWID, * FROM reported_change%s' % suffix)
        for r in results:
            print("Id: %d" % r[0])
            print("Product: %s" % r[1])
            print("Version: %s" % r[2])
            print("Object Type: %s" % r[3])
            print("Action: %s" % r[4])
            print("Data: %s" % r[5])
            print("Module: %s" % r[6])
            print('')

    def show_options_url(self, *param):
        suffix = ''
        if param[0] != '':
            params = param[0].split('-')
            if len(params) > 1 :
        ...
        results = self.query('SELECT ROWID, * FROM url%s' % suffix)
        for r in results:
            print("Id: %d" % r[0])
            print("Product: %s" % r[1])
            print("Version: %s" % r[2])
            ...
            print("Url: %s" % r[4])
            if r[5] != None:
                print("Data: %s" % base64.b64decode(r[5]))
            print('')

    def show_qa_issue(self, *param):
        suffix = ""
        ...
        query = 'SELECT ROWID, * FROM qa_issue%s' % suffix
        ...
        for r in results:
            ...
            print("Product: %s" % r[1])
            print("Version: %s" % r[2])
            print("File Name: %s" % r[3])
            print("Location: %s" % r[4])
            print("Line No: %s" % r[5])
            print("Line: %s" % r[6].rstrip())
            print("Note: %s" % r[7])
            print("Module: %s" % r[8])
            print('')
    ...
    def complete_load(self, text, *ignored):
        return [x for x in Framework._loaded_modules if text in x]
